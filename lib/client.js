...
/**
 * Fetches the metadata placement data managed by electric-boray
 */
BorayClient.prototype.getPlacementData = function getPlacementData(cb) {
    assert.func(cb, 'cb');
    ...
    if (rpcctx) {
        ...
        rpc.rpcCommonBufferData({
            ...
            if (placementData.length !== 1) {
                var err_msg = 'bad server response: expected 1 JSON object, found %d';
                err = new VError(err_msg, placementData.length);
                callback(err);
                return;
            }
