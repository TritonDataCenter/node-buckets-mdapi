...
FastConnection.prototype.onSocketConnect = function onSocketConnect() {
    /*
     * It's important to enable TCP KeepAlive on connections to the BucketsMdapi
     * server so that we can identify connections that have failed as a result
     * of a remote system panic, power cycle, power off, or a network partition.
     ...
     * TCP_KEEPALIVE_ABORT_THRESHOLD).  Since BucketsMdapi is only used inside
...
/*
 * Used by the rest of the BucketsMdapi client
 */
