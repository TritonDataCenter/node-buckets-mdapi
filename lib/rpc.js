 ...
 *           function).
 *
 *     rpcCommonSingleMessage(args, callback)
 *
 *           Makes an RPC using the arguments specified by "args", reads and
 *           buffers data returned by the RPC, waits for the RPC to complete,
 *           and then invokes "callback" as:
 ...
 *               callback(err, data)
 *
 *           where "err" is an error if the RPC fails, or 0 or more than 1
 *           messages are recieved.  If the RPC succeeds, "data" is the first
 *           and only piece of data received.
 *
 ...
 *           way.
 *
 * All of these functions take care of unwrapping errors if the buckets-mdapi
 * client was configured that way.  Named arguments are:
 *
 *     rpcmethod, rpcargs, log,     See FastClient.rpc() method.
 *     ignoreNullValues
 *
 *     rpcctx                       buckets-mdapi's "rpcctx" handle, a wrapper
 *                                  around a FastClient that includes context
 *                                  related to this buckets-mdapi client.
 *     wrapErrors                   {Boolean} Whether error messages in
 *                                  received Fast messages should be wrapped as
 *                                  VError objects and returned as errors to
 *                                  the caller.  Defaults to false.
 *
 * Error messages are considered to be objects with an "error" key, which
 * itself is an object that must contain a "name" and "message" property.
 */
function rpcCommon(args, callback) {
    ...
    req.once('error', function (err) {
        if (rpcctx.unwrapErrors()) {
            ...
            'buckets-mdapi client ("%s" to "%s")', addrs.local, addrs.remote);
        }
        callback(err);
    });
...
function rpcCommonSingleMessage(args, callback) {
    ...
    req = rpcCommonBufferData(args, function (err, data) {
        ...
        if (data.length !== 1) {
            err = new VError('bad response: expected 1 message, found %d',
                data.length);
...
/*
 * See the "unwrapErrors" constructor argument for the BucketsMdapiClient.
 */
